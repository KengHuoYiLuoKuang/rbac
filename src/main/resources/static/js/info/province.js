define(function () {
  return function () {
    var dg = $("#province_dg");
    var searchFrom = $("#province_search_from");
    var form;

    // 使用edatagrid，需要而外导入edatagrid扩展
    dg.datagrid({
      url: '/info/province/list',
      emptyMsg: "还未创建省份信息",
      idField: "provinceCode",
      fit: true,
      rownumbers: true,
      fitColumns: true,
      border: false,
      ...
      columns: [[{
      ...
      },
        {
          ...
          formatter: function (value, row, index) {
            return authToolBar({
              "member-edit": '<a data-id="' + row.provinceCode + '" class="ctr ctr-edit">编辑</a>',
              "member-delete": '<a data-id="' + row.provinceCode + '" class="ctr ctr-delete">删除</a>'
            }).join("");
          }
        }
      ]],
      ...
      dg.datagrid("getPanel").on('click', "a.ctr-edit", function () {// 编辑按钮事件
          ...
          $.messager.confirm("删除提醒", "确认删除此省份信息?", function (r) {
              if (r) {
                  $.get("/info/province/delete", {id: id}, function () {
                      // 数据操作成功后，对列表数据，进行刷新
                      dg.datagrid("reload");
                  });
              }
              ...
              buttons: [{
                  ...
                  handler: function () {
                      if (form.form('validate')) {
                          $.post("/info/province/save", form.serialize(), function (res) {
                              dg.datagrid('reload');
                              dialog.dialog('close');
                          })
                      }
